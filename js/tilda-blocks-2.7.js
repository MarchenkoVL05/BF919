function t835_init(recid) {
  ...
  if (form.getAttribute('formactiontype') != 1 && !specCommentInput) {
          form.insertAdjacentHTML('beforebegin' ,'<div style="position: absolute; left: -5000px; bottom:0;"><input type="text" name="form-spec-comments" value="Its good" class="js-form-spec-comments"  tabindex="-1" /></div>')
  }
...
function t835_setError(rec, quizQuestionNumber) {
  ...
  if (errorsTypeObj != undefined) {
          ...
          var errorTextCustom = rec.querySelector('.t835 .t-form').querySelector('.t-form__errorbox-middle').querySelector('.js-rule-error-' + errorType).textContent;
          var sError = '';
          if (errorTextCustom != '') {
  ...
  for (var i = 0; i < quizQuestion.length; i++) {
        ...
        if (quizQuestionNumber >= quizQuestion.length - maxCountOfCaptureFields) {
                ...
                if (quizFormWrapper.classList.contains('t835__quiz-form-wrapper_newcapturecondition')) {
                        isCaptureGroup = quizQuestion[i].classList.contains('t-input-group_cb') || quizQuestion[i].classList.contains('t-input-group_em') || quizQuestion[i].classList.contains('t-input-group_nm') || quizQuestion[i].classList.contains('t-input-group_ph')
                }
                if (isCaptureGroup) {
...
function t835_disabledPrevBtn(rec, quizQuestionNumber) {
  var prevBtn = rec.querySelector('.t835__btn_prev');
  quizQuestionNumber == 0 ? prevBtn.setAttribute('disabled', !0) : prevBtn.setAttribute('disabled', !1)
}
